...
const Navbar = ({ currentUser }) => {
    ...
    return (
        <nav className='flex bg-primary w-full py-4 px-6 items-center justify-between'>
            ...
            {navModal && (
                <div className='absolute right-0 top-16 mr-2 lg:hidden bg-[#25292c] text-primaryText rounded-lg shadow-lg p-4 z-10'>
                    <div className='flex flex-col justify-center items-center gap-3'>
                        ...
                        </Link>
                        {
                            user?.data ? (
                                ...
                                <Link href='/signup' className='cursor-pointer hover:brightness-90 w-full'>
                                    <div className='flex items-center gap-2 bg-secondaryText py-2 px-3 rounded-lg justify-center'>
                                        Sign In
                                    </div>
                                </Link>
                                )
            ...
            <Menu as="div" className="relative lg:flex text-left flex-1 justify-end hidden">
                {user?.data ? (
                    <>
                        ...
                        >
                            <Menu.Items className="absolute top-12 right-0 mt-2 w-36 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                                <div className="p-3 ">
                                    <Menu.Item>
                                        {({ active }) => (
                    ...
                    (
                        <Link href='/signup'>
                            <button>
                                <div className="flex items-center justify-center px-6 py-2 text-lg text-white bg-blueText rounded-md hover:bg-opacity-80 focus:outline-none">
                                    Sign Up / Login
                                </div>
                            </button>
                        </Link>
